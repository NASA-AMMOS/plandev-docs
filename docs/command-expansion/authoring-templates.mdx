...
To create a new sequence template for an activity type (or to edit existing ones) navigate to the **Sequence Templates** page of SeqDev, in the "Sequencing" category.

<figure>
  <img alt="SeqDev UI - Templates Page" src={templatePage} />
...
- a [parcel ID](/sequencing/editor/#parcel) (to get the set of allowed commands from the dictionary)
- a [mission model ID](/mission-modeling/introduction) (to enable access to the activity types)
...
<figure>
  <img alt="PlanDev UI - Create New Template" src={createNewTemplate} />
...
<figure>
  <img alt="PlanDev UI - Template Editor" src={templateEditor} />
...
During expansion, PlanDev goes through your plan looking for activities which have a template associated with them, and compiles the template with the following input data structure from the activity, which may be used in the template:
...
PlanDev defines 4 helpers for use in templates. User-defined helpers are not yet supported, so only these 4 are available at the moment.
