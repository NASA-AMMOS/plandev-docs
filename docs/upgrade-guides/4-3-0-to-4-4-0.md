...
* The [TypeScript Command Expansion EDSL](https://github.com/NASA-AMMOS/plandev/issues/1863) has been removed in favor of [Sequence Templates](/command-expansion/template-expansion/introduction.md).
...
All [Java packages](https://github.com/NASA-AMMOS/plandev/packages?q=plandev&ecosystem=maven) and classes across PlanDev and SeqDev have moved from:
...
These projects must be updated and rebuilt for PlanDev 4.4.0. You may do this before upgrading your deployment, but do not upload the rebuilt JARs until PlanDev itself has been upgraded to 4.4.0. 
To update an existing project (model, goal or constraint):
...
2. Rename any Java service-loader files under `src/main/resources/META-INF/services` that contain the old namespace. For Mission Models, this typically includes:
   ...
   ```
3. Check your project's `build.gradle` for explicit `aerie` dependencies or repository URLs and update them to the corresponding `plandev` names if present.
...
PlanDev 4.4.0 removes support for the TypeScript Command Expansion EDSL. [Sequence Templates](/command-expansion/template-expansion/introduction.md) are now the supported command-expansion workflow.
...
Users of the TypeScript Expansion EDSL should migrate their expansion logic to [Sequence Templates](/command-expansion/template-expansion/introduction.md). See [Removing TypeScript Command Expansion](https://github.com/NASA-AMMOS/plandev/issues/1863) for additional details about this change.
