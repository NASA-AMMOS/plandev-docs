...
<RemoteCodeBlock
  language="java"
  showLineNumbers
  title="PlanDev Mission Model Template - CollectData.java"
  url="https://raw.githubusercontent.com/NASA-AMMOS/plandev-mission-model-template/main/missionmodel/examples/CollectData.java"
></RemoteCodeBlock>

PlanDev automatically generates parameter serialization boilerplate for every activity type defined in the mission model's [package-info.java](../introduction.mdx#the-package-infojava-file).
...
In order for PlanDev to detect an activity type, its class must be annotated with the `@ActivityType` tag. An activity type is declared with its name using the following annotation:

```java
@ActivityType("CollectData")
```

By doing so, the PlanDev annotation processor can discover all activity types declared in the mission model, and validate that activity type names are unique.

## Activity Type Metadata

Metadata of activities are structured such that the PlanDev annotation processor can extract this metadata given particular keywords. Currently, the annotation processor recognizes the following tags: `contact`, `subsystem`, `brief_description`, and `verbose_description`.
