...
| `-e ENV_PATH`, `--env-path ENV_PATH`                                     | The path to the `.env` file used to deploy PlanDev. **Must** define `AERIE_USERNAME` and `AERIE_PASSWORD`. Defaults to `.env`. |
...
```sh
> aerie_db_migration.py -a -db PlanDevMerlin --all
###########
PlanDevMerlin
###########
VERSION        TYPE  NAME
...
```sh
> aerie_db_migration.py -a -db PlanDevMerlin
###############################
