...
If you're already familiar with Kubernetes, feel free to jump to [Within PlanDev Context](/plandev-docs/deployment/advanced-kubernetes/#within-aerie-context)
...
This will remove all PlanDev `Pods`, but preserve the `aerie-dev` namespace and `Secret` we created earlier, since we created those ad-hoc, not from a manifest. This allows you to quickly iterate on manifests.
