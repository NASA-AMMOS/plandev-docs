...
User Roles are sets of permissions that define how a user is allowed to interact with the PlanDev system.
By default, PlanDev provides three User Roles:
...
:::info
The `viewer` role is the most restricted role PlanDev supports. To ensure PlanDev works as expected when using your new role,
...
Currently, all functions are in the PlanDevMerlin Database.
...
For example, `{"name":{"_like":"plandev--%"}}` means that the row being inserted must have its `name` field begin with "plandev--".
...
For example, `{"name":{"_like":"plandev--%"}}` means that the row must have its `name` field begin with "plandev--" after being updated.
This is optional.
...
#### Action, Function, and Workspace Permissions

In addition to the coarse-grained permissions granted by the Hasura Metadata,
certain Functions, Actions, and Workspace operations have what are known as fine-grained permissions.
For Functions and Actions, **it is not sufficient** to only allow the role to see the operation via the Hasura metadata.
Workspace operations are checked by the Workspace server against the same role permission table.
This section will go over how to specify these fine-grained permissions
for a custom role.

Action, Function, and Workspace Permissions are represented as three JSONs.
The structure of these JSONs are `{ KEY_1: PERMISSION, KEY_2: PERMISSION, ... , KEY_N: PERMISSION }`.
The absence of a Key will be considered equivalent to a role **NOT** having permission to perform the Action, Function, or
Workspace operation.
...
| Key | Hasura Action |
| -: | :- |
| `assign_activities_by_filter` | assignActivitiesByFilter
...
<details closed>
<summary>Keys Permitted for Workspace Permissions</summary>

| Key | Meaning |
| -: | :- |
| `create_workspace` | Create a workspace |
| `delete_workspace` | Delete a workspace |
| `list_workspace_contents` | List workspace files and folders |
| `read_file_directory` | Read a workspace file or directory |
| `write_file_directory` | Create or write a workspace file or directory |
| `delete_file_directory` | Delete a workspace file or directory |

`create_workspace` is coarse-grained. If present, it must be set to `NO_CHECK`. All other workspace permission keys can
use the workspace permission values listed below.
...
<details closed>
<summary>Values Permitted for Workspace Permissions</summary>

| Permission | Meaning |
| -: | :- |
| `NO_CHECK` | The user may perform the workspace operation with no ownership or collaborator check |
| `OWNER` | The user must be the workspace owner |
| `COLLABORATOR` | The user must be a workspace collaborator |
| `OWNER_COLLABORATOR` | The user must be either the workspace owner or a workspace collaborator |

</details>

Once you have devised your Action, Function, and Workspace Permissions JSONs,
you can use this GraphQL Mutation to apply them to your custom role:

```graphql
mutation updateUserRolePermissions($function_permissions: jsonb!, $action_permissions: jsonb!, $workspace_permissions: jsonb!) {
...
The first time a user not in the system logs in, they will be added to the PlanDev system with a set of User Roles they
