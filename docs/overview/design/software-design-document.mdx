...
This document details the current state of the design of the PlanDev system.
The document defines the components which make up the PlanDev system and how these relate to the higher level system architecture.
Additionally, design rationale and trade studies are described to provide guidance and support for design
decisions. Where available, the many PlanDev component interface
definitions and relevant published software interface specifications will be provided inline or referenced.

## Overview

PlanDev is an open-source extensible software system originally built for planning, scheduling, and commanding space missions.
Initially developed by NASA's [Advanced Multi-Mission Operations System (AMMOS)](https://ammos.nasa.gov/), PlanDev provides modeling and simulation capabilities that can be used for mission planning and analysis during project formulation all the way through operations, where it can be used to manage and validate spacecraft activity plans.

PlanDev provides low-code solutions for authoring scheduling rules to autogenerate plans, authoring and evaluating constraints to assist with plan validation, and authoring logic to expand activities into sequences of commands for execution. Sequences can also be authored and edited independently following an open source sequencing specification, seq-json, that includes a variety of commanding styles (e.g. absolute, relative, command completion).

As a multi-tenant system, PlanDev allows multiple distributed users to collaborate in real-time on a single plan or concurrently work on multiple plans for multiple missions. Additionally, PlanDev's service-based architecture allows for efficient system deployment and scalability on the cloud.

## References

If you are interested in learning how to use PlanDev, a great place to start is our [Fast Track](https://nasa-ammos.github.io/plandev-docs/introduction/#fast-track). More detailed information on [planning](https://nasa-ammos.github.io/plandev-docs/category/planning/), [scheduling](https://nasa-ammos.github.io/plandev-docs/scheduling/introduction/), [sequencing](https://nasa-ammos.github.io/plandev-docs/category/sequencing/), and [command expansion](https://nasa-ammos.github.io/plandev-docs/command-expansion/introduction/) is available throughout our online documentation.

If you are interested in learning how to build and configure models to use with PlanDev, please refer to our [Mission Modeling](https://nasa-ammos.github.io/plandev-docs/mission-modeling/introduction/) documentation.

PlanDev's design requirements are currently documented in the PlanDev Software Requirements Document (SRD) delivered to AMMOS (DOC-002388 Rev. B). If you are interested in receiving a copy of this document, please contact plandev-support@googlegroups.com.

Detailed code documentation is also available in our [Java Docs](https://nasa-ammos.github.io/plandev-docs/java-docs/introduction/) section.
...
- **Mission model** - The term "adaptation" has become significantly
  ...
  As a result PlanDev has chosen to use the term mission model to denote
  ...
  which run as an PlanDev simulation. It's specific as to the purpose of
...
## PlanDev System Design

### System Architecture

This section describes the PlanDev system architecture, the guiding
...
import c4PlandevContext from './assets/c4-plandev-context.png';

<figure>
  <img alt="PlanDev Context" src={c4PlandevContext} style={{ maxHeight: '100%', width: '700px' }} />
  <figcaption>Figure 1: PlanDev Context</figcaption>
</figure>

PlanDev is designed to be incrementally evolvable - so that it can more
...
For example the
[merlin-worker](https://github.com/NASA-AMMOS/plandev/tree/develop/merlin-worker)
is a part of the Merlin bounded context. Much of PlanDev has been
...
import c4PlandevContainer from './assets/c4-plandev-container.png';

<figure>
  <img alt="PlanDev Container" src={c4PlandevContainer} style={{ maxHeight: '100%', width: '700px' }} />
  <figcaption>Figure 2: PlanDev Composition at the Container Level</figcaption>
</figure>
...
- **PlanDev UI** - The server provides a user interface which supports
...
- **Common Access Manager (CAM)** - PlanDev delegates authentication and
...
- **Automated Scheduler** - An automated scheduler operates external
  to the PlanDev deployment and carries out computer aided mission
  activity scheduling by issuing queries and mutations against the
  PlanDev GraphQL API. As a component external to the PlanDev deployment,
...
The PlanDev GraphQL API presents a consistent application boundary to
PlanDev users. The API server enables the composition of multiple APIs
...
1. **Tooling API Management** - GraphQL is a newer technological
   approach to APIs (2012).<br /> **Mitigation:** PlanDev has chosen to
...
1. **Client-API Loose Coupling** - Each new client application must
   ...
   mutations as made possible by the PlanDev GraphQL schema.<br />
   **Mitigation:** None. In the PlanDev context this is not considered a
   benefit.
...
Javalin is extremely simple in its implementation and use. The PlanDev
...
PlanDev simulation. The driving use cases for the merlin worker
architecture are:

1. Executing multiple concurrent simulations
   - PlanDev is a multi-user application and needs to be able to execute
...
1. Separate process spaces to provide
   ...
   - The ability to reduce simulation time by executing simulations on
     computationally performant hardware, while running other PlanDev
...
process among the various PlanDev components. While PlanDev in general can
...
merlin-worker's responsibilities into a separate process enables PlanDev
...
PlanDev has been designed with the DDD principle of bounded
...
PlanDev's datastores include both relational databases and the file
system. A large majority of PlanDev's data is considered to be
...
appropriate choice when dealing with structured data. PlanDev also uses
the filesystem to store mission model .JAR files uploaded to PlanDev.
...
- Design and selection criteria for the db
  ...
  - Historically missions are insufficiently equipped in both the
    ...
    IT infrastructures. As such, it is advantageous for PlanDev to
...
- Backups and Migration
  ...
  - Plans, constraints, simulation resource profiles - all data in the
    ...
    and migration between PlanDev versions.

### Deployment

PlanDev deployment is designed to accommodate two user groups; the
individual installation on a local machine and the installation on
hosted infrastructure.

Each PlanDev service is deployed as a Docker container with the
...
the container name. PlanDev service containers have their IP and port
...
PlanDev UI are proxied through the aerie-ui service.

Identity data is provided by configuring CAM policies. The PlanDev
GraphQL API Server uses CAM identity data combined with user roles defined
in the database, to manage authorization of API access and limit access
per API field granularity.

### Data Model

#### Datasets

In PlanDev, a dataset is a set of data describing several associated
...
PlanDev utilizes a number of EDSLs for parts of the system that require
...
The concept of the PlanDev Command Expansion service took the best parts
...
code execution. This is the PlanDev TS User Code Runner that now
...
The PlanDev approach to simulation aims to support both activity plan
simulation (APGen) and sequence simulation (SeqGen). To do this the
PlanDev simulation architecture must built upon a general description of
...
detailed information about how PlanDev accomplishes this.
...
windows where certain conditions are met. An PlanDev constraint is a
...
expressions are defined by the PlanDev constraint grammar. This grammar
is defined in a Typescript eDSL that, when executed, produces a JSON
AST which can be interpreted by Merlin.

In the PlanDev constraint AST, the internal nodes represent the
...
PlanDev uses a meta-programming approach to generate additional source
...
injection code for various PlanDev data objects. PlanDev uses
...
PlanDev chose to use Java's annotations processing mechanism. Unlike a
...
In PlanDev the scheduling agent "Automated Scheduler" exists as a
separate entity and makes queries and mutation against the PlanDev
API. As shown in the Figure below, an PlanDev developed and maintained
automated scheduler is a part of the standard PlanDev deployment. The
PlanDev Automated Scheduler makes queries of simulation results,
constraint evaluations, and makes plan mutations via the PlanDev
API. This loose coupling is the same as any third party entity which
may query the PlanDev API gateway. As a result, a mission can choose to
forgo the PlanDev Automated Scheduler and implement their own scheduler
and scheduling algorithm, which makes use of the same PlanDev API.
...
particularly burdensome restrictions. The loose coupling of the PlanDev
...
- Extensibility - extensibility (definition of new constraint concepts
  without having to modify any part of PlanDev or a mission model)
...
<figure>
  ...
  <figcaption>Figure 4: The PlanDev Scheduling Component</figcaption>
</figure>

When executing the Automated Scheduler is likely to make many calls to
the PlanDev API. The Automated Scheduler will sensibly batch API calls
to reduce unnecessary communication round trips. A scheduler will need
to be able to make the following requests of the PlanDev API:
...
1. The Context diagram shows the software system (PlanDev) in its
   broadest scope.
