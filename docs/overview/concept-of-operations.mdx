...
PlanDev tools and services allow systems and services to be customized to meet different mission and user needs. For instance, PlanDev simulations can be configured to run at different fidelity levels depending on the planners' need at different steps of the planning process. Or a PlanDev deployment can be customized to accommodate many parallel simulation and scheduling requests scaled for usage. Similarly PlanDev UI can be customized by the mission system or by the users interactively to display data relevant for different contexts.
