import simulationInconsQuery from './assets/simulation_incons_query.mp4';
...
import simulateIncons from './assets/simulate_with_incons.mp4';
...
Navigate to your deployment's Hasura GraphQL console by selecting `GraphQL Console` in the dropdown under `PlanDev`.
By default, the GraphQL console is accessible on [port 8080](http://localhost:8080).

<figure>
  <img alt="PlanDev UI - Dropdown Menu Highlighting GraphQL Console Option" src={gqlDropdown} />
...
Next, navigate to your deployment's Gateway by selecting `Gateway` in the dropdown under `PlanDev`.
By default, the Gateway is accessible on [port 9000](http://localhost:9000).

<figure>
  <img alt="PlanDev UI - Dropdown Menu Highlighting Gateway Option" src={gatewayDropdown}/>
...
The most important element of this JSON is `filename`. It contains the name of the file within PlanDev.
...
where `<path-to-file-store>` is the path to where the `plandev_file_store` volume is mounted in Merlin,
and `<filename>` is the value of `"filename"` from the earlier JSON from the Gateway.

By default, `<path-to-file-store>` is `merlin_file_store`.
