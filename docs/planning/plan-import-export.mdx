...
PlanDev has the ability to export plans into a JSON-based file format, and import plans from files using the same format.
This allows you to save plans you've created in PlanDev for backup or archival purposes, transfer plans between different PlanDev
deployments, or interface with other tools which may ingest or modify the plan file.

PlanDev plan files are mainly a way to persist **activities, tags, anchors, and arguments** from a plan.
...
The format of these files is described by this Typescript type in the PlanDev repository:

https://github.com/NASA-AMMOS/plandev-gateway/pull/97/files#diff-aadcd2e92cdf1b17122b6fc65396f3e5603e14064d730a020b231cd30134704e

Future releases of PlanDev will include a JSON schema fully describing the format.

## Exporting a plan

To export an PlanDev plan, first open it in PlanDev, then open the "Plan Metadata" panel. Use the export button in the top
right of the panel to export a `plan.json` file.

<figure>
  <img alt="PlanDev UI - Export Plan from Plan Metadata panel" src={exportPlanScreenshot} />
</figure>

## Importing a plan

On the Plans page, you can select an PlanDev plan file to upload when creating a new plan. This will pre-populate
