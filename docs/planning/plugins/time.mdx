...
Time input and presentation in the PlanDev UI can be customized by implementing a time plugin. Currently the time plugin is only used on the plans and individual plan page. This time plugin does not affect how time is stored in the database and instead converts times from the database format to the plugin format on the fly in the UI.
...
- The Time Plugin must be named `time-plugin.js` and must be placed inside of the `static/resources` directory of the PlanDev UI.
...
| `ticks.maxLabelWidth`   | <code>number</code>                                                                                           | The maximum width in pixels of any tick label. Used internally by the PlanDev UI timeline to compute the number of ticks that should be displayed in the timeline. <br/> <i>Default: 130</i>                                                                                         |
...
The Time Plugin is responsible for gracefully failing when exceptions are encountered within the plugin. When an operation such as `parse` or `format` fails, the plugin should return `null` to indicate to the PlanDev UI that an error occurred and that the date is invalid.

## Example Plugins

Below is a basic example that sets the additional time formats to a single format that displays the current UTC year. For additional plugin examples, please refer to the [aerie-ui-plugin-examples](https://github.com/NASA-AMMOS/plandev-ui-plugin-examples) repository.
