...
import setAnchorsDemo from './assets/anchoring_ui_demo.mp4';
import removeAnchorsDemo from './assets/remove_anchors_demo.mp4';
...
For ease of use, PlanDev allows anchors that are not able to be simulated to exist. These anchors are considered to be invalid.
...
<figure>
  <img alt="PlanDev UI - Anchor Validation Error in a Tooltip" src={invalidAnchorTooltip} />
  <figcaption>Figure 1: PlanDev UI - Anchor Validation Error in a Tooltip</figcaption>
</figure>

To see a full list of the invalid anchors for a plan, select the `Anchor Validation Errors` tab from the Error Console

<figure>
  <img alt="PlanDev UI - Anchor Validation Errors in the Error Console" src={invalidAnchorsList} />
  <figcaption>Figure 2: PlanDev UI - Anchor Validation Errors in the Error Console</figcaption>
</figure>
...
<div>
  <p>
    Currently, it is impossible to delete an activity directive that has other directives anchored to it via the UI. If
    you would like to delete an activity directive that has other directives anchored to it, either manually remove the
    anchors from the dependent directives or <Link to="/api/examples/planning/anchors">use the API</Link> for more
    options.
  </p>
</div>
