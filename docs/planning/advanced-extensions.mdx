...
Extensions are external tools that can be invoked directly from inside of PlanDev. When an Extension is called, the following values are passed along in the request body:
...
- `message`: the message to be displayed as a notification inside of PlanDev
...
When looking at a plan in PlanDev, all registered Extensions can be found under the Navbar entry between Scheduling and View.
...
<figure>
  <img alt="PlanDev Planning Navbar - Extensions" src={extensions} />
  <figcaption>Figure 1: PlanDev Planning Navbar - Extensions</figcaption>
</figure>
