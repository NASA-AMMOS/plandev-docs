...
With this in mind, let's create a new file in `missionmodel/src/main/java/missionmodel` directory called `DataModel.java` with an empty class definition (we will eventually instantiate this new class within the `Mission` class, but that isn't necessary now).
