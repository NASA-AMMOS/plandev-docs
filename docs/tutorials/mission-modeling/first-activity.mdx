...
Since activity types are classes in Java, create a new class called `CollectData` and add the following Java annotation above that class, which allows PlanDev to recognize this class as an activity type.
...
Next, we need to tell our activity how and when to effect change on the `RecordingRate` resource, which is done in an [Activity Effect Model](https://nasa-ammos.github.io/plandev-docs/mission-modeling/activity-types/effect-model/).
