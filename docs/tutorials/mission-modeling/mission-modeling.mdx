...
Before we begin writing modeling code, make sure that you have access to a PlanDev deployment as we'll be loading our model into PlanDev to build plans and view simulation results. You can deploy PlanDev locally on your machine by following the simple steps outlined in our [Fast Track](https://nasa-ammos.github.io/plandev-docs/introduction/#fast-track) instructions. Once you have checked that the PlanDev UI is available on [http://localhost/](http://localhost/), you should be ready proceed with this tutorial.
