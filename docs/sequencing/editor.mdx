...
SeqDev Actions can read and update workspace file metadata through the `plandev-actions` API:
...
tools that manage workspace files outside the SeqDev UI. See the [`WorkspaceBindings` implementation](https://github.com/NASA-AMMOS/plandev/blob/develop/workspace-server/src/main/java/gov/nasa/ammos/plandev/workspace/server/WorkspaceBindings.java) for full endpoint behavior.
